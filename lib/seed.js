...
// Legacy admin account (kept for backward compat)
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@alga.et';
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'admin123';

// Platform owner — the only account that can approve providers
...
function run() {
  ...
  if (!data.users.some((u) => u.email === ADMIN_EMAIL)) {
    ...
    changed = true;
    console.log(`  Legacy admin -> ${ADMIN_EMAIL} / ${ADMIN_PASSWORD}`);
  }

  // Platform owner account
  if (!data.users.some((u) => u.email === OWNER_EMAIL)) {
    data.users.push({
      id: nanoid(10),
  ...
  // Always print owner credentials on boot
  const ownerPwd = process.env.OWNER_PASSWORD || 'changeme123';
  console.log(`\n  Owner login -> ${OWNER_EMAIL} / ${ownerPwd}`);
  console.log(`  Owner phone -> ${OWNER_PHONE}\n`);
}

module.exports = { run, ADMIN_EMAIL, OWNER_EMAIL };
