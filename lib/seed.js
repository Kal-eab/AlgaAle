...
async function run() {
  ...
  // The email may already belong to an older account (SUPPORT_EMAIL was reused);
  // promote it rather than silently leaving it without support access.
  const supportExists = await store.findUserByCredential(SUPPORT_EMAIL);
  if (supportExists && supportExists.role !== 'support') {
    await store.updateUserRole(supportExists.id, 'support');
    console.log(`  Promoted ${SUPPORT_EMAIL} from ${supportExists.role} to support`);
  }
  if (!supportExists) {
    await store.createUser({
      id: nanoid(10),
